...
class Prompter
{
    ...
    SetUpDefaultText()
    {
        ...
        this.DefaultText.push("sometimes thinking logical pattern can get tedious. take some rest ");
    ...
    DrawEvent()
    {
        if ( !prompt.ineventmode)
            return;
        console.log(prompt.Canvas.box.h); // 38.625
        ...
        var dimfromcanv = prompt.Canvas.box.h;
        // Blinking effect
        if ( prompt.eventtimer % 2 == 0 )
        {
            g.fillStyle = 'rgb(255,255,255)';
            //'rgb(255,50,211)';//'rgb(0,0,255)';
            g.font = dimfromcanv+"px Fontlab";
...
function InitPrompt()
{
    ...
    newDiv.id = "prompt_div";
    var canv = document.createElement('canvas');
    canv.id = 'prompt';
    canv.tabIndex = 1000;
    formone.appendChild(newDiv); 
    newDiv.appendChild(canv);
    var PromptCanvas = new Canvas(canv,newDiv, 
                               new Box(EditorCanvas.box.x,(window.innerHeight-window.innerHeight/24-20),EditorCanvas.box.w, window.innerHeight/24 ));
    prompt = new Prompter(PromptCanvas);
}